...
func (s *service) AddSchedule(ctx context.Context, schedule model.Schedule) (int, error) {
...
func (s *service) GetScheduleWithIntake(ctx context.Context, userID int, scheduleID int) (model.Schedule, error) {
    reqID := middleware.GetReqID(ctx)
    ...
    if schedule.ID == 0 {
        ...
        return model.Schedule{}, fmt.Errorf("schedule_id is 0: %w", err)
    }
...
func (s *service) GetNextTakings(ctx context.Context, userID int) ([]model.Schedule, error) {
    reqID := middleware.GetReqID(ctx)
